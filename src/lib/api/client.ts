...
function buildUrl(path: string, query?: QueryParams, settings = getApiSettings()) {
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  const settings = options.settings || getApiSettings();
  const apiKey = settings.apiKey.trim() || getApiKey();
...
export function createProductMediaForm(file: File, options?: { descricao?: string; textoAlternativo?: string }) {
...
export function createAudioForm(file: Blob, diaDeVendaId?: UUID | null) {
  const form = new FormData();
  form.append("file", file, `venda-${new Date().toISOString()}.webm`);
...
export const api = {
  ...
  dias: {
    ...
    create: (body: CriarDiaDeVendaRequest) =>
      apiRequest<DiaDeVenda>("/api/v1/dias-de-venda", { method: "POST", body }),
